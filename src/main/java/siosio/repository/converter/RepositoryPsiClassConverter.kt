...
open  class RepositoryPsiClassConverter : PsiClassConverter() {
  ...
  override fun getScope(context: ConvertContext): GlobalSearchScope? {
    return context.module?.let {
      val includeTest = !GlobalSearchScope.moduleRuntimeScope(it, false).contains(context.file.originalFile.virtualFile)
      GlobalSearchScope.moduleRuntimeScope(it, includeTest)
